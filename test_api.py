...
def test_api_book_by_author1():
    resp = requests.get(
        f"{base_url}search/product?phrase=Карл Маркс", headers=headers)
...
def test_api_book_by_author2():
    resp = requests.get(
        f"{base_url}search/product?phrase=Капитал", headers=headers)
    assert resp.status_code == 200
...
def test_api_book_by_author3():
    resp = requests.get(
        f"{base_url}search/product?phrase=English Grammar", headers=headers)
    assert resp.status_code == 200
...
def test_api_book_by_author4():
    resp = requests.get(
        f"{base_url}search/product?phrase=пустой ответ", headers=headers)
    assert resp.status_code == 200
...
def test_api_book_by_author5():
    resp = requests.get(
        f"{base_url}search/product?phrase=цена должна быть числом", headers=headers)
    assert resp.status_code == 200
...
def test_invalid_endpoint():
    response = requests.get(
        f"{base_url}search/invalid_endpoint?phrase=тест", headers=headers)
